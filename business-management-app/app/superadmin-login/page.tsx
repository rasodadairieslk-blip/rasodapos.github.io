  ...
  async function handleLogin(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (username === 'superadmin' && password === 'superadmin') {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('superadmin-auth', 'true');
      }
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-12 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-md overflow-hidden rounded-[2rem] border border-slate-700 bg-slate-900/95 p-8 shadow-soft">
        <h1 className="text-3xl font-semibold text-white">Superadmin login</h1>
        <p className="mt-3 text-slate-400">Enter the superadmin credentials to manage outlets and users.</p>
