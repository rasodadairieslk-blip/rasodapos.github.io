...
function updateUserBadge() {
    ...
    const roleDisplay = state.currentUser.role === 'superadmin' ? 'Superadmin' : state.currentUser.role.charAt(0).toUpperCase() + state.currentUser.role.slice(1);
    selectors.userBadge.textContent = `${state.currentUser.username} • ${roleDisplay}`;
}
...
async function handleLogin(event) {
    ...
    const superadminSection = document.getElementById('superadminLoginSection');
    const isSuperadminTab = !superadminSection.classList.contains('hidden');

    if (isSuperadminTab) {
        const username = document.getElementById('superadminUsername').value.trim();
        const password = document.getElementById('superadminPassword').value;
        if (username === 'superadmin' && password === 'superadmin') {
            state.currentUser = { username: 'superadmin', password: 'superadmin', role: 'superadmin', id: 'superadmin' };
            localStorage.setItem('milkyHugCurrentUser', JSON.stringify(state.currentUser));
            selectors.loginScreen.classList.add('hidden');
            updateUserBadge();
            await renderDashboard();
            showSection('outletConfigSection');
            toggleAdminFeatures();
            document.getElementById('superadminUsername').value = '';
            document.getElementById('superadminPassword').value = '';
        } else {
            loginError.classList.remove('hidden');
            loginError.textContent = 'Invalid superadmin credentials.';
        }
    } else {
        ...
        if (!user) {
            loginError.classList.remove('hidden');
            loginError.textContent = 'Login failed. Please verify your username and password.';
            return;
        }
        ...
        toggleAdminFeatures();
        selectors.usernameInput.value = '';
        selectors.passwordInput.value = '';
    }
}

function switchLoginTab(tab) {
    const posLoginSection = document.getElementById('posLoginSection');
    const superadminLoginSection = document.getElementById('superadminLoginSection');
    const posLoginTab = document.getElementById('posLoginTab');
    const superadminLoginTab = document.getElementById('superadminLoginTab');

    if (tab === 'pos') {
        posLoginSection.classList.remove('hidden');
        superadminLoginSection.classList.add('hidden');
        posLoginTab.classList.add('border-sky-600');
        posLoginTab.classList.remove('border-transparent');
        posLoginTab.classList.add('text-slate-900');
        posLoginTab.classList.remove('text-slate-400');
        superadminLoginTab.classList.remove('border-sky-600');
        superadminLoginTab.classList.add('border-transparent');
        superadminLoginTab.classList.remove('text-slate-900');
        superadminLoginTab.classList.add('text-slate-400');
    } else {
        posLoginSection.classList.add('hidden');
        superadminLoginSection.classList.remove('hidden');
        superadminLoginTab.classList.add('border-sky-600');
        superadminLoginTab.classList.remove('border-transparent');
        superadminLoginTab.classList.add('text-slate-900');
        superadminLoginTab.classList.remove('text-slate-400');
        posLoginTab.classList.remove('border-sky-600');
        posLoginTab.classList.add('border-transparent');
        posLoginTab.classList.add('text-slate-400');
        posLoginTab.classList.remove('text-slate-900');
    }
}
...
function toggleAdminFeatures() {
    ...
    const outletConfigMenuBtn = document.getElementById('outletConfigMenuBtn');

    if (isSuperadmin) {
        if (outletConfigMenuBtn) outletConfigMenuBtn.classList.remove('hidden');
        ...
        if (userButton) {
            ...
            userButton.classList.add('opacity-40');
        }
    } else {
        if (outletConfigMenuBtn) outletConfigMenuBtn.classList.add('hidden');
        document.querySelectorAll('.edit-product-btn, .delete-product-btn, #openAddProduct, #openReceiptTemplate, #emailConfigForm button, #userForm button, .reset-user-btn, .delete-user-btn').forEach((button) => {
...
document.getElementById('posLoginTab')?.addEventListener('click', () => switchLoginTab('pos'));
document.getElementById('superadminLoginTab')?.addEventListener('click', () => switchLoginTab('superadmin'));
